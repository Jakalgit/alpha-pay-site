...
import styles from "@/styles/components/client/PricingForm.module.css"
import stylesHome from "@/styles/pages/index.module.css"
	...
	return (
		<div className={`relative flex ${styles.tariffsBlock}`}>
			...
			<div className={`flex flex-col ${styles.left}`} style={{zIndex: 1}}>
				<h3 className={`golos-text text-[color:--gold-color-1] font-semibold ${styles.pricingHeadText}`}>
					{t("section5.pricing")}
				</h3>
			...
			<div
				...
				style={{zIndex: 1}}
			>
				<div className={`!border border-[color:--purple-color-6] bg-[color:--purple-color-3] ${styles.tableContainer}`}>
						...
						{tableColumn.map((row, index) => (
							<tr key={index} style={{display: "inline-flex"}}>
								...
								{currencySolutions?.length < 6 && (
									<>
										{Array(5 - currencySolutions?.length).fill(0).map((_, i) =>
											<td key={`td_1_${i}`}></td>
										)}
									</>
								)}
