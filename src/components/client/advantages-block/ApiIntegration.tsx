...
import styles from "@/styles/components/client/adavntages-block/ApiIntegration.module.css"
...
const ApiIntegration = () => {
	...
	const codeLines = [
		{
			text: "curl -X POST \"https://alfa-pay.org/api\" \\",
			spaces: 4
		},
		{
	...
	return (
		<div className={`flex flex-col bg-[color:--purple-color-3] ${stylesBlock.descriptionBlock} ${styles.wrapper}`}>
			<h4 className={`golos-text text-[color:--gold-color-1] font-semibold m-0 ${stylesBlock.largeText}`}>
				{t('head')}
			</h4>
			<div className={`flex ${styles.block}`}>
				<div className="flex flex-col">
					...
					<div className={styles.downloadButtonWrapper}>
						<button
							className={`geologica text-[color:--purple-color-1] flex items-center justify-center ${stylesBlock.smallText}`}>
							{t('download')}
							<DownloadIcon className="fill-[color:--purple-color-1]"/>
						</button>
					</div>
				</div>
				<div className="flex justify-end">
					<div className={`flex flex-col !border border-[color:--purple-color-5] bg-[color:--foreground-color] ${styles.codeBlock}`}>
						<div className="flex">
							<div className={`geologica bg-[color:--purple-color-1] text-[color:--foreground-color] ${styles.codeType}`}>
								cURL
							</div>
						</div>
