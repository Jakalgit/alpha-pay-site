...
import styles from "@/styles/components/client/adavntages-block/PersonalAccount.module.css"
	...
	return (
		<div className={`flex flex-col bg-[color:--purple-color-3] ${stylesBlock.descriptionBlock}`}>
			...
			<div className={`flex flex-wrap ${styles.blocks}`}>
				{icons.map((icon, i) =>
					<div key={i} className={`flex items-center bg-[color:--foreground-color] !border border-[color:--purple-color-1] ${styles.block}`}>
						{React.cloneElement(icon, { className: "fill-[color:--purple-color-1]" })}
						<p className={`geologica text-[color:--purple-color-1] ${stylesBlock.smallText}]`}>
							{t(`p${i + 1}`)}
						</p>
					</div>
