...
import styles from "@/styles/components/client/HomeAdvantages.module.css"
...
const HomeAdvantages: React.FC<IProps> = ({ countries }) => {
			...
			<div className={`grid ${styles.advantagesWrapper}`}>
				{blocks.map(({id, name, imgTop, block, classNameBlock, classNameSquare}) =>
					<React.Fragment key={id}>
						<div className={`flex relative overflow-hidden bg-[color:--foreground-color] ${styles.block} ${classNameSquare}`}>
							<img
							...
							<div className="flex flex-col items-center w-full z-1">
								...
								<div className="flex justify-center">
									<button
										className={`flex items-center geologica text-[color:--gold-color-1] duration-300 hover:text-white
								group ${styles.buttonFontSize} ${styles.buttonConnect}`}
									>
										{t("connect")}
										<ArrowIcon
											className="transition-colors duration-300 fill-[color:--gold-color-1] group-hover:fill-white"
										/>
									</button>
								</div>
								<div className="flex justify-center mt-auto">
									<button
										onClick={() => expand(id)}
										className={`flex items-center geologica text-[color:--purple-color-1] !border border-[color:--purple-color-1] 
											duration-300 group hover:bg-[color:--purple-color-1] hover:text-black ${styles.buttonFontSize} ${styles.buttonExpand}`}
									>
										{t("expand")}
										<ArrowIcon
